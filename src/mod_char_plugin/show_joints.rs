...
pub fn cubes_names_on_joints(
    ...
    let font_handle = asset_pack
        .font_files
        .get("fira_sans.ttf")
        .expect("That the font exists");

    // This loop will interact through the scene entities that were loaded
    // And display their bones and their names good debugger
        ...
        for entity in all_entities_with_children.iter_descendants(scene_entity) {
            if let Err(_) = mesh_handles.get(entity) {
                if let Ok(_) = global_transforms.get(entity) {
                    ...
                    // THis will make the joint the parent of the text
